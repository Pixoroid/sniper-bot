...
def calc_size(btc_price, balance):
...
# ══════════════════════════════════════════
# ✅ TRADE MONITOR
# Balance हो = Real trade
# Balance न हो = Tracking only
# ══════════════════════════════════════════
def trade_monitor(pid, side, entry, sl,
    ...
    print(f"🎯 TP1:    {tp1} (+300 pts) R:R 1:1")
    print(f"🎯 TP2:    {tp2} (+600 pts) R:R 1:2")
    print(f"🎯 TP3:    {tp3} (+900 pts) R:R 1:3")
    ...
    for _ in range(5000):
        ...
        if side == "buy":
            ...
            if not tp1_hit and cp >= tp1:
                ...
                if has_balance and pid:
                    ...
                    place_order(pid, close_side,
                                ...
                                "stop_loss_order")
                print(f"\n{'✅'*8}")
                print(f"TP1 HIT! +300 pts R:R 1:1")
                ...
                if has_balance and pid:
                    ...
                    place_order(pid, close_side,
                                ...
                                "stop_loss_order")
                print(f"\n{'🚀'*8}")
                print(f"TP2 HIT! +600 pts R:R 1:2")
                print(f"SL → +300 Lock:{current_sl:.0f}")
                print(f"{'🚀'*8}\n")
                ...
                if has_balance and pid:
                    ...
                    place_order(pid, close_side,
                                ...
                                "stop_loss_order")
                print(f"\n{'🔥'*8}")
                print(f"TP3 HIT! +900 pts R:R 1:3")
                print(f"SL → +600 Lock:{current_sl:.0f}")
                print(f"{'🔥'*8}\n")
            ...
            if cp <= current_sl:
                ...
                final_pts = round(
                    current_sl - entry, 1)
                daily_pnl_inr += final_pts * \
                    (size * INR_RATE
                     if has_balance else 0.001)
                rr_f = round(
                    final_pts / sl_dist, 2) \
                    if sl_dist > 0 else 0
                ...
                if final_pts > 0:
                    print(f"🎯 PROFIT EXIT! "
                          f"{'💰 REAL' if has_balance else '👁️ TRACKING'}")
                else:
                    print(f"🛑 SL HIT! "
                          f"{'💰 REAL' if has_balance else '👁️ TRACKING'}")
                print(f"{'='*45}")
                ...
                print(f"✅ TP1:     "
                      f"{'HIT ✅' if tp1_hit else 'Miss ❌'}")
                print(f"✅ TP2:     "
                      f"{'HIT ✅' if tp2_hit else 'Miss ❌'}")
                print(f"✅ TP3:     "
                      f"{'HIT ✅' if tp3_hit else 'Miss ❌'}")
