...
def calc_size(btc_price, balance_usd):
    ...
    size         = max(size, 0.001)
    print(f"💰 ${balance_usd:.4f}×{LEVERAGE}x→{size} BTC")
    return size
...
def candles(resolution="15m", limit_hours=12):
    try:
        ...
        if data:
            ...
            print(f"📊 {resolution} Candles:{len(data)} Last:{last_close}")
        return data
    except Exception as e:
        print(f"❌ Candles:{e}")
        return None
...
def find_divergence(candle_data):
    ...
    if len(price_lows) >= 2 and len(rsi_lows) >= 2:
        ...
        if (p_low2 < p_low1
                ...
                and swing_size >= 500):
            print(f"🟢 BULLISH DIV! Swing:{swing_size:.0f}")
            return "BUY"
    ...
    if len(price_highs) >= 2 and len(rsi_highs) >= 2:
        ...
        if (p_hi2 > p_hi1
                ...
                and swing_size >= 500):
            print(f"🔴 BEARISH DIV! Swing:{swing_size:.0f}")
            return "SELL"
    ...
    if (move > 1.5 and current_rsi < 45 and red and vol_good):
        print(f"🔴 TREND SELL! Move:{move:.1f}% RSI:{current_rsi:.1f}")
        return "SELL"

    if (move < -1.5 and current_rsi > 55 and green and vol_good):
        print(f"🟢 TREND BUY! Move:{move:.1f}% RSI:{current_rsi:.1f}")
        return "BUY"
...
def trade_monitor(pid, side, entry, sl, size, has_balance):
    global sl_count, consecutive_sl, daily_pnl_inr, last_signal
    ...
    tp1 = round(entry + 300, 1) if side == "buy" else round(entry - 300, 1)
    tp2 = round(entry + 600, 1) if side == "buy" else round(entry - 600, 1)
    tp3 = round(entry + 900, 1) if side == "buy" else round(entry - 900, 1)
    ...
    print(f"💲 Entry:  {entry}")
    print(f"🛡️  SL:     {current_sl} ({sl_dist:.0f} pts)")
    ...
    for _ in range(5000):
        ...
        print(f"{'💰' if has_balance else '👁️'} "
              f"CP:{cp:.0f} | P&L:{profit_pts:+.0f}pts | "
              f"R:R 1:{rr:.1f} | SL:{current_sl:.0f} | Trail:{trail_dist:.0f}")
