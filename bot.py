...
# ✅ 1. Time Filter — IST
TRADE_TIMES = [
    (9, 0, 12, 0),    # 9:00 AM - 12:00 PM
    (19, 0, 23, 0),   # 7:00 PM - 11:00 PM
]
...
# ✅ 8. Daily Profit/Loss Track
DAILY_PROFIT_TARGET = 500   # ₹500 profit → बंद
DAILY_LOSS_LIMIT    = 300   # ₹300 loss → बंद
daily_pnl_inr       = 0.0
INR_RATE            = 85
...
def is_trade_time():
    ...
    for (sh, sm, eh, em) in TRADE_TIMES:
        ...
        if start <= curr <= end:
            ...
            return True
    print(f"⏰ Trade Time नहीं: {h:02d}:{m:02d} IST")
    return False
...
def signal(candle_data):
    ...
    print(f"🕯️ P:{p:.1f} Green:{green} "
          f"Vol:{vol_good} 1hr:{trend_1h} ATR:{current_atr:.1f}")

    buy_cross  = prev_e9 < prev_e50 and curr_e9 > curr_e50
    sell_cross = prev_e9 > prev_e50 and curr_e9 < curr_e50

    if buy_cross and green and vol_good and trend_1h == "BULL":
        print("🟢 BUY CONFIRMED!")
        return "BUY", dynamic_sl

    elif sell_cross and red and vol_good and trend_1h == "BEAR":
...
def trail_monitor(pid, side, entry, size, sl_pts):
    ...
    # ✅ Partial size — 25% each TP
    s1 = round(size * 0.25, 6)  # 25% TP1
    s2 = round(size * 0.25, 6)  # 25% TP2
    s3 = round(size * 0.25, 6)  # 25% TP3
    s4 = round(size * 0.25, 6)  # 25% Trail
    remaining = size

    print(f"🔄 Entry:{entry} SL:{current_sl}")
    print(f"🎯 TP1:{tp1}(25%) TP2:{tp2}(25%) "
          f"TP3:{tp3}(25%) Trail:25%")
    ...
    for _ in range(2000):
        ...
        profit_inr = round(profit_pts * remaining * INR_RATE, 1)
        ...
        if side == "buy":
            ...
            if not tp1_hit and cp >= tp1:
                ...
                place_order(pid, close_side, "limit_order",
                            remaining, current_sl, current_sl,
                            "stop_loss_order")
                pnl = round(TP1_PTS * s1 * INR_RATE, 1)
                daily_pnl_inr += pnl
                print(f"✅ TP1! 25% Exit +₹{pnl} "
                      f"SL→Breakeven Rem:{remaining}")

            # TP2 — 25% exit + Lock 300
                ...
                daily_pnl_inr += pnl
                print(f"🚀 TP2! 25% Exit +₹{pnl} "
                      f"Lock+{TP1_PTS} Rem:{remaining}")

            # TP3 — 25% exit + Lock 600
                ...
                daily_pnl_inr += pnl
                print(f"🔥 TP3! 25% Exit +₹{pnl} "
                      f"Lock+{TP2_PTS} Rem:{remaining}")
            ...
            # Trail 25%
                ...
                if new_sl > current_sl:
                    ...
                    cancel_all_orders(pid)
                    place_order(pid, close_side, "limit_order",
                                remaining, current_sl,
                                current_sl, "stop_loss_order")
                    print(f"🚀 Trail SL:{current_sl}")

            # SL Hit
            if cp <= current_sl and remaining > 0:
                cancel_all_orders(pid)
                pnl = round(profit_pts * remaining * INR_RATE, 1)
                daily_pnl_inr += pnl
                if tp1_hit:
                    print(f"🎯 Exit! +{profit_pts}pts "
                          f"₹{pnl:+.0f} 🔥")
                    consecutive_sl = 0
                else:
...
def order(side, dynamic_sl):
    try:
        ...
        print(f"🛡️ ATR SL:{sl} OK:{sl_res.get('success')}")
...
def run():
    ...
    print("⚙️  Time Filter + Partial Exit + "
          "Consecutive SL + Daily P&L")
    ...
    while True:
        ...
        # ✅ 8. Daily Profit Target Hit
        if daily_pnl_inr >= DAILY_PROFIT_TARGET:
            print(f"🎯 Daily Target Hit! "
                  f"₹{daily_pnl_inr:.0f} Trading बंद 🔒")
            time.sleep(3600)
            continue

        # ✅ 8. Daily Loss Limit Hit
        ...
        # ✅ 4. Consecutive SL Protection
        if consecutive_sl >= MAX_CONSECUTIVE_SL:
            print(f"⛔ {MAX_CONSECUTIVE_SL} Consecutive SL! "
                  f"2hr Rest 😴")
            consecutive_sl = 0
            time.sleep(7200)
            continue
